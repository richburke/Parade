 ...
 * - Memoization
 * - Write faileds to file
 * - Read in past faileds
 * 
 * Next steps:
...
var G = {"attempts":[], "faileds":[], "newFaileds":[]};
...
function writeArrayToFile(a, fn) {
    var s = '';
    __.each(a, function(elem, indx){s += indx==0 ? elem : os_.EOL+elem;});

    if (fs_.existsSync(fn)) {
        fs_.appendFileSync(fn, s);
    } else {
        fs_.writeFileSync(fn, s);
    } 
}

function readFileIntoArray(fn, a) {
    if (!fs_.existsSync(fn)) return;

    var s = fs_.readFileSync(fn, 'utf8');
    return a.concat(s.split(os_.EOL));
}

function getFileSize(fn) {  
    var fstat = fs_.statSync(fn);
    return fstat.size;
}
...
function hashDeck(deck) {
    var hash = '';
    for (var i=0; i < deck.length; i++) {
        hash += getCardAbbreviationByValue(deck[i]);
    }
    return hash;
}

function isDeckInArray(deck, a) {
    var hashedDeck = hashDeck(deck);
    return __.contains(a, hashedDeck);
}
...
G.faileds = readFileIntoArray('faileds.txt', G.faileds);
console.log('initialized with '+G.faileds.length+' faileds.');
console.log('file size '+Math.floor(B2mB(getFileSize('faileds.txt')))+'mB.');
console.log('starting...');
var res = move(originalState, 0);
console.log(res);
console.log(translateValueDeckToAbbreviations(res.state));
console.log('unique attempts: '+G.attempts.length);
console.log('new faileds: '+G.newFaileds.length);
console.log('total faileds: '+G.faileds.length);
writeArrayToFile(G.newFaileds, 'faileds.txt');
console.log('...done.');
